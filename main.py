...
body_sub = body[['FAMILY_ID_rcd','entry_lat','entry_lon','exit_lat','exit_lon','UNITBED_START_DT','UNITBED_END_DT','GENDER','RACE','ETHNICITY','HOMELESSNESS_PRIMARY_REASON','PRIOR_ZIP','EXIT_ZIP']]
...
# def duration_stay(body):
# 	#Duration of stay
# 	end=[]
# 	for date in body['UNITBED_END_DT']:
# 		try:
...
# 	#Duration of stay
# 	start=[]
# 	for date in body['UNITBED_START_DT']:
# 		try:
...
#2013 Only
fil=[]
for a,i in enumerate(body_sub['start']):
	if 'year' in i.keys():
			fil.append(i.year)
	else:

body_sub['year'] = body_sub['UNITBED_START_DT'].map(lambda k: k.year() if k.keys() in 'year' else 0)
...
def transition_convert(body):
	transition = body.groupby(['entry_lat','entry_lon','exit_lat','exit_lon']).size().reset_index()
	#Rid of \N's
	...
	# transition['filter_lon_exit'] = body_sub['exit_lon'].map(lambda k: 1 if k < boundary[1][1] and k > boundary[1][0] else 0 )
...
def borough_and_year_group(data):
	...
	survival.to_csv('dhs_survival_data.csv',sep='\t',index=None)
	return(gdf)
